...
def database_creation(oferts,filename):
    ...
    print(df)
    print("-----------------------------------------------------")
    print(f"The data has been saved to a file {filename}.xlsx")

def welcome():
    #Przywitanie użytkownika oraz ewentualne pominięcie ofert z konkretnym wymaganiem
    print("Enter the name of the xlsx file to which you want to save the database")
    filename=input("> ")

    print("Enter a skill that excludes a job offer, if there is more, separate them with a comma (Skills like: linux, java, python...)")
    skillsy = input("> ")
    skillsy = skillsy.split(",")

    return skillsy,filename


skillsy,filename = welcome()
while True:
    print("Processing...")

    oferts = job_searching(skillsy)
    database_creation(oferts,filename)
